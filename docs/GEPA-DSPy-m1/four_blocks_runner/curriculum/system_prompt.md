...
These notes exist for human reviewers and for GEPA's reflection LM, which
benefits from knowing what each section is *trying to do* — so it doesn't
mutate a section into something that defeats its purpose.

- **Section 1 (Identity)** establishes voice. Failure mode it prevents:
  the model drifting into therapist-pose, life-coach-pose, or
  recovery-influencer-pose. The "thoughtful friend who has studied" frame
  is the load-bearing image.
- **Section 2 (What You See)** narrows the diagnostic surface. Failure
  mode: the model trying to address every complaint as a unique problem.
  Everything routes to one of four buckets or a mix.
- **Section 3 (How You Listen)** gives substantive cues so the model can
  identify the block by *language*, not by self-report. Failure mode:
  asking the user "what emotion are you feeling?" before answering.
- **Section 4 (Four Lenses)** is the core IP — the framework rendered as
  interpretive moves rather than named concepts. Each lens is paired
  with a "your move" so the model knows what action follows from
  recognition. Failure mode: the model recognizing the pattern and
  then naming it (instant violation of Section 6).
- **Section 5 (How You Respond)** is the deterministic shape: validate →
  reflect → plant → invite → stop. This is the response sequence GEPA
  will most likely try to mutate; the numbered structure protects each
  step from being silently dropped.
- **Section 6 (What You Never Do)** is the brand-protection layer.
  These constraints exist because the brand has explicitly chosen the
  paraphrased layer for user-facing surfaces. Failure mode: the model
  citing Ellis or saying "this is awfulizing" because that's what
  pretrained CBT data wants it to do.
- **Section 7 (Safety Override)** is non-negotiable and must always
  outrank curriculum. GEPA should not mutate this section's *intent*;
  it may rephrase wording.
- **Section 8 (Style)** captures the felt sense of the voice and gives
  the model permission to use the four verbatim Parr lines, which are
  the rare cases where source language is conversational enough to
  pass through the brand layer untouched.
- **Section 9 (Examples)** anchors all of the above in concrete shape.
  These are deliberately tiny and follow the validate → reflect → plant
  → invite arc explicitly, so they double as in-prompt few-shot
  exemplars and as a reference for the per-block expected_behavior
  rubrics.

### Alignment with per-block expected_behavior rubrics

The per-block golden examples test specific competencies that this prompt
is built to deliver:

- **Anger rubric** will test for: did the model gently expose a Should /
  Must / Demand without naming it? Did it separate the spark (event)
  from the fire (emotion)? Did it acknowledge the hurt before
  re-framing? → Sections 4 (Lens 3, Lens 4), 5 (steps 2–4), 9 (Anger
  example).
- **Anxiety rubric** will test for: did the model anchor the user back
  into the present, surface the catastrophic story, and avoid
  reassurance? → Sections 4 (Lens 2c, Lens 4 — "I can't stand it"), 5,
  9 (Anxiety example).
- **Depression rubric** will test for: did the model separate event
  from self-rating, refuse hopelessness without dismissing it, and
  invite a small, ordinary action? → Sections 4 (Lens 1, Lens 4 —
  rating), 5, 9 (Depression example).
- **Guilt rubric** will test for: did the model separate the act from
  the self, refuse identity-collapse language, and invite repair
  rather than self-punishment? → Sections 4 (Lens 4 — rating), 8
  (Kornfield-via-Parr line), 9 (Guilt example).

The "never overt" directive from the user is enforced structurally by
Section 6's framework-name blocklist plus Section 4's "use; never name"
header — the model is asked to *do the work* of the framework while
*sounding like* a friend.

---

## Variant placeholders (for GEPA reflection)

GEPA's reflection LM is most likely to mutate the following substrings.
Markers help reflection focus there without scrambling structure.

- **`{{IDENTITY_VOICE_DESCRIPTOR}}`** — Section 1, the phrase "warm,
  brief, present, and non-judgmental." Likely site of voice
  experimentation. Keep four-ish adjectives.
- **`{{LENS_NAMES}}`** — Section 4, the pattern names ("the story vs.
  the situation," "the three quiet truths," "spark, story, fire,"
  "patterns that cook us"). Internal labels only — never said to the
  user — but renaming may help or hurt the model's ability to *use*
  them. Strong mutation candidate.
- **`{{LISTEN_CUES_*}}`** — Section 3 cue phrases per block. Adding,
  removing, or rephrasing cues is a likely high-impact mutation site.
- **`{{RESPONSE_SEQUENCE}}`** — Section 5's six numbered steps. The
  *order* is load-bearing; the *wording* of each step is mutable.
- **`{{NEVER_LIST}}`** — Section 6's blocklist of forbidden terms.
  Adding terms is safe; removing terms breaks the brand layer.
- **`{{VERBATIM_QUOTES}}`** — Section 8's quotable lines. They are
  brand-approved; do not mutate the quoted text itself, only the
  permission language around it.
- **`{{EXAMPLES}}`** — Section 9's four worked examples. Whole-example
  rewriting is a strong mutation site; the validate → reflect → plant
  → invite arc inside each is structural.

Sections 1, 2, 5 (numbered structure), 7, and the six-step response
sequence in 5 are **structural** — GEPA should preserve their shape.
Sections 3, 4 (lens descriptions), 6 (entries on the never-list), 8
(style hints), and 9 (examples) are **textual** — wording-level mutation
is welcome.
